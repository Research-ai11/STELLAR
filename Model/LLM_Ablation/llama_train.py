"""
Training script for STELLAR next-POI prediction (LLaMA backbone).

This script follows the same structure/style as the main (clean) train.py:
...
Notes:
  - Do NOT hardcode HF tokens in code. Use environment variables instead:
      export HF_TOKEN=...
  - This file assumes your unified `param_parser.py` provides --backbone llama
    and that `model.py` includes PFA_LLAMA.
"""
...
def prepare_run(args) -> Any:
    ...
    # Match your main train.py layout: runs/train/<data>/<logit_mode>/<name>...
    save_dir = Path(
        increment_path(
            Path(args.project) / args.data / str(args.logit_mode) / args.name,
...
def resolve_data_paths(dataset_key: str) -> DataPaths:
    ...
    if dataset_key == "nyc":
        return DataPaths(
            ...
            geohash_embedding="../../data/nyc/graph/nyc_geohash_gat_space_embedding_llama.csv",
    ...
    if dataset_key == "tky":
        return DataPaths(
            ...
            geohash_embedding="../../data/tky/graph/tky_geohash_gat_space_embedding_llama.csv",
...
def build_models_and_optim(
    *,
    ...
    poi_encoder = POIEncoderGCN(
        ...
        dropout=getattr(args, "gcn_dropout", 0.5),
    ).to(device)
...
def main() -> None:
    ...
    # Ensure this script is used with the LLaMA preset
    # (You can remove this guard if you want a single train.py for all backbones.)
    if getattr(args, "backbone", None) not in (None, "llama"):
        logging.warning(f"--backbone={args.backbone} given, but this train script is for LLaMA. Continuing anyway.")
